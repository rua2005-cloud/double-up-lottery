(() => {
  ...
  const timeAttack = $('timeAttack'), timerEl = $('timerValue'), timeSub = $('timeSub');
  ...
  const hintLabel = $('hintLabel') || document.querySelector('.hint-label'), hintText = $('hintText'), hintSub = $('hintSub'), hintBox = $('hintBox'), auraWrap = $('auraWrap');
  ...
  function clearHint() {
    ...
    hintSub.textContent = atActive ? '次のAT当選後に新しいAT示唆が表示されます。' : '当選すると次のダブルアップ示唆が表示されます。';
    renderAura(2);
  }
  ...
  async function doDoubleUp() {
    ...
    if (win) {
      ...
      } else messageEl.textContent = '失敗。今回の賞金は0になりました。示唆は確実ではありません。';
